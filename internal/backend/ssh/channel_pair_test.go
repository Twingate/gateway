...
func TestChannelPair_ServePanicClosesChannels(t *testing.T) {
	...
	// ChannelPair.close() instead of leaking the accepted channel.
	channels := newProxyChannels(t, "session")
...
// proxyChannels is the channel-layer fixture: an ChannelPair serving one channel between two
...
// newPair builds the ChannelPair over the proxy-held ends, applying the fixture's logger,
