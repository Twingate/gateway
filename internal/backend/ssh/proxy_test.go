...
func TestProxy_UpstreamFailures(t *testing.T) {
	...
	tests := []struct {
		name    string
		setup   func(t *testing.T, sshProxy *Proxy) string
		wantErr string
	}{
		{
		...
		},
		{
			...
			setup: func(t *testing.T, _ *Proxy) string {
				t.Helper()
...
// newProxyConn wraps conn in the frontend.ProxyConn the proxy serves, with test GAT claims and
// the address of the upstream the proxy dials.
func newProxyConn(conn net.Conn, upstreamAddr string) *frontend.ProxyConn {
	upstreamHost, port, _ := net.SplitHostPort(upstreamAddr)
	upstreamPort, _ := strconv.Atoi(port)

	proxyConn := frontend.NewProxyConn(conn, nil, nil, zap.NewNop(),
		frontend.CreateProxyConnMetrics(prometheus.NewRegistry()))
...
// end and the server end wrapped as the frontend.Conn the proxy serves; upstreamAddr is the
// upstream the proxy dials.
func newDownstreamConn(t *testing.T, upstreamAddr string) (client net.Conn, server *frontend.ProxyConn) {
	t.Helper()
...
// testListener is a real loopback listener presenting accepted connections as the frontend.Conn
...
type panickingConn struct{ frontend.Conn }
...
func dialDownstream(t *testing.T, sshProxy *Proxy, clientConn net.Conn) (*ssh.Client, error) {
	t.Helper()
...
func connCount(sshProxy *Proxy) int {
	sshProxy.mu.Lock()
