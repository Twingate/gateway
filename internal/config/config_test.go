...
func TestTLSDynamicCAConfig_Validate(t *testing.T) {
	selfSign := &TLSSelfSignCAConfig{CertificateFile: "ca.crt", PrivateKeyFile: "ca.key"}
	vault := &TLSVaultCAConfig{VaultConfig: VaultConfig{Address: "https://vault:8200"}, Role: "gateway"}
...
func TestTLSVaultCAConfig_Validate(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid",
			cfg:  TLSVaultCAConfig{VaultConfig: VaultConfig{Address: "https://vault:8200"}, Role: "gateway"},
		},
		{
		...
		{
			name:        "missing role",
			cfg:         TLSVaultCAConfig{VaultConfig: VaultConfig{Address: "https://vault:8200"}},
			wantErr:     true,
			...
			ssh: SSHConfig{
				...
				CA: SSHCAConfig{
					Vault: &SSHCAVaultConfig{
						VaultConfig: VaultConfig{Address: "https://vault:8200"},
						Role:        "gateway",
					},
				},
			},
...
func TestSSHCAVaultConfig_Validate(t *testing.T) {
	...
	t.Run("missing role (no override roles)", func(t *testing.T) {
		cfg := &SSHCAVaultConfig{VaultConfig: VaultConfig{Address: "https://vault:8200"}}
		err := cfg.Validate()
		require.Error(t, err)
	...
	t.Run("valid with top-level role", func(t *testing.T) {
		cfg := &SSHCAVaultConfig{VaultConfig: VaultConfig{Address: "https://vault:8200"}, Role: "gateway"}
